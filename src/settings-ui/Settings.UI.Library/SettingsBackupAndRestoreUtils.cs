        ...
        public (bool Success, string Message, string Severity) RestoreSettings(string appBasePath, string settingsBackupAndRestoreDir)
        {
            try
            {
                ...
                foreach (var currentFile in backupSettingsFiles)
                {
                    ...
                    if (currentSettingsFiles.TryGetValue(currentFile.Key, out var settingsFilename))
                    {
                        // we have a setting file to restore to
                        var currentSettingsFileJson = GetExportVersion(backupRetoreSettings, currentFile.Key, settingsFilename);
                        ...
                        if (JsonNormalizer.Normalize(settingsToRestoreJson) != JsonNormalizer.Normalize(currentSettingsFileJson))
                        {
                            ...
                            var newCurrentSettingsFile = JsonMergeHelper.Merge(File.ReadAllText(settingsFilename), settingsToRestoreJson);
                            File.WriteAllText(settingsFilename, newCurrentSettingsFile);
            ...
            lock (backupSettingsInternalLock)
            {
                ...
                try
                {
                    ...
                    foreach (var currentFile in currentSettingsFiles)
                    {
                        ...
                        var doBackup = false;
                        if (lastBackupSettingsFiles.TryGetValue(currentFile.Key, out var settingsFilename))
                        {
                            // there is a previous backup for this, get an export version of it.
                            var lastSettingsFileDoc = GetExportVersion(backupRetoreSettings, currentFile.Key, settingsFilename);
