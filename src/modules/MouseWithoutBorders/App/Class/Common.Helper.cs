    ...
    internal partial class Common
    {
                ...
                while (true)
                {
                    ...
                    else
                    {
                        if (Common.SwitchLocation.Count > 0)
                        {
                            ...
                            Common.Log($"+++++ Moving mouse to {Common.SwitchLocation.X}, {Common.SwitchLocation.Y}");

                            // MaxXY = 65535 so 100k is safe.
                            if (Common.SwitchLocation.X > XY_BY_PIXEL - 100000 || Common.SwitchLocation.Y > XY_BY_PIXEL - 100000)
                            {
                                InputSimu.MoveMouse(Common.SwitchLocation.X - XY_BY_PIXEL, Common.SwitchLocation.Y - XY_BY_PIXEL);
                            }
                            else
                            {
                                InputSimu.MoveMouseEx(Common.SwitchLocation.X, Common.SwitchLocation.Y);
                            }
            ...
            if (!Common.RunOnLogonDesktop && !Common.RunOnScrSaverDesktop)
            {
                ...
                if (Setting.Values.HideMouse && Setting.Values.StealFocusWhenSwitchingMachine && Common.SendMessageToHelper(0x407, new IntPtr(left), new IntPtr(top), true) == 0)
                {
                    try
                    {
                        ...
                        if (mouseClick)
                        {
                            InputSimu.MouseClickDotForm(left + 1, top + 1);
                        }
                    }
                    catch (Exception e)
                    {
        ...
        internal static void RunDDHelper(bool cleanUp = false)
        {
            ...
            if (h.ToInt32() <= 0)
            {
                ...
                // Common.CreateLowIntergrityProcess("\"" + Path.GetDirectoryName(Application.ExecutablePath) + "\\MousewithoutBordersHelper.exe\"", string.Empty, 0, false, 0);
        ...
        internal static int SendMessageToHelper(int msg, IntPtr wparam, IntPtr lparam, bool wait = true, bool log = true)
        {
            ...
            if (log)
            {
                Common.Log($"SendMessageToHelper: HelperWindow={h}, Return={rv}, msg={msg}, w={wparam.ToInt32()}, l={lparam.ToInt32()}, Post={!wait}");
            }
        ...
        internal static void OpenHelpLink()
        {
            _ = CreateProcessInInputDesktopSession(
                $"\"{Path.GetDirectoryName(Application.ExecutablePath)}\\{HelperProcessName}.exe\"",
                NativeMethods.IsRunningAtMicrosoft() ? "help-in" : "help-ex",
                GetInputDesktop(),
                0);
        }
        ...
        internal static bool GetUserName()
        {
            if (string.IsNullOrEmpty(Setting.Values.Username) && !Common.RunOnLogonDesktop)
            {
                ...
                Common.Log("[Username] = " + Setting.Values.Username);
            }
