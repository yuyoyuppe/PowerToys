...
namespace MouseWithoutBorders
{
    ...
    internal partial class Common
    {
            ...
            else
            {
                if (x < primaryScreenBounds.Left + skipPixels)
                {
                    ...
                    else
                    {
                        ...
                        {
                            RequestedXEx = primaryScreenBounds.Left;
                            RequestedYEx = y;
                            return true;
                        }
                    }
            ...
            else
            {
                if (x > primaryScreenBounds.Right - skipPixels)
                {
                    ...
                    else
                    {
                        ...
                        {
                            RequestedXEx = primaryScreenBounds.Right;
                            RequestedYEx = y;
                            return true;
                        }
                    }
            ...
            else
            {
                if (y < primaryScreenBounds.Top + skipPixels)
                {
                    ...
                    else
                    {
                        ...
                        {
                            RequestedXEx = x;
                            RequestedYEx = primaryScreenBounds.Top;
                            return true;
                        }
                    }
                }
            ...
            else
            {
                if (y > primaryScreenBounds.Bottom - skipPixels)
                {
                    ...
                    else
                    {
                        ...
                        {
                            RequestedXEx = x;
                            RequestedYEx = primaryScreenBounds.Bottom;
                            return true;
                        }
                    }
                }
        ...
        internal static void RemoveDeadMachines()
        {
            ...
            // DoSomthingInUIThread(UpdateMenu);
        ...
        private static string AddToMachinePool(DATA package)
        {
            ...
            if (Common.MachinePool.TryFindMachineByName(name, out MachineInf machineInfo))
            {
                ...
                {
                    Log("AddToMachinePool: Des ID updated: " + DesMachineID.ToString() + "/" + package.Src.ToString());
            ...
            else
            {
                ...
                {
                    Log("AddToMachinePool: could not add a new machine: " + name);
        ...
        {
            Log("UpdateClientSockets: " + logHeader);
            Sk?.UpdateTCPClients();
        }
        ...
        {
            Common.Log("========== BEGIN THE SETUP EXPERIENCE ==========", true);
        ...
        internal static void ShowMachineMatrix()
        {
            ...
            {
                TelemetryForEvent?.TrackEvent("SForm");
        ...
        private static string[] LiveMachineMatrix
        {
            get
            {
                ...
                Log($"Matrix: {string.Join(",", MachineMatrix)}, Connected: {string.Join(",", connectedMachines)}");
        ...
        internal static void SendMachineMatrix()
        {
            ...
            for (int i = 0; i < MachineMatrix.Length; i++)
            {
                ...
                package.Type = PackageType.Matrix
                    ...
                    | (Setting.Values.MatrixOneRow ? 0 : PackageType.MatrixtworFlag);
                ...
                Log($"matrixedMachine sent: [{i + 1}]:[{MachineMatrix[i]}]");
            }
        }
        ...
        internal static void UpdateMachineMatrix(DATA package)
        {
            ...
            string matrixedMachine = package.MachineName;

            if (i is > 0 and <= MAX_MACHINE)
            {
                Log($"matrixedMachine: [{i}]:[{matrixedMachine}]");

                MachineMatrix[i - 1] = matrixedMachine;

                if (i == MAX_MACHINE)
                {
                    Setting.Values.MatrixCircle = (package.Type & PackageType.MatrixswapFlag) == PackageType.MatrixswapFlag;
                    Setting.Values.MatrixOneRow = !((package.Type & PackageType.MatrixtworFlag) == PackageType.MatrixtworFlag);
            ...
            {
                Log("Invalid machine Matrix package!");
            }
        }
        ...
        internal static void SwitchToMultipleMode(bool multipleMode, bool centerScreen)
        {
            if (multipleMode)
            {
                TelemetryForEvent?.TrackEvent("MMode");
