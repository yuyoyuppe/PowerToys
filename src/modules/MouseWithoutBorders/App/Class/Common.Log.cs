...
namespace MouseWithoutBorders
{
    public class Thread
    {
        ...
        internal static void SuppendAllThreadsBut(int threadId)
        {
            lock (ThreadsLock)
            {
...
#pragma warning restore 618
    ...
    {
        private static readonly string[] AllLogs = new string[MAXLOG];
        ...
        {
            TelemetryClient logTelemetry = TelemetryForLog;
            int logCount = LogCounter.AddOrUpdate(log, 1, (key, value) => value + 1);

            if (logTelemetry != null)
            {
                if (RepeatedLogIndexSelection.Contains(logCount))
                {
                    logTelemetry.TrackTrace(log, severityLevel, new Dictionary<string, string>
                    {
                        { "Cnt", logCount.ToString(CultureInfo.InvariantCulture) },
                        { "TiO", ((long)(DateTime.UtcNow - LastResumeSuspendTime).TotalSeconds).ToString(CultureInfo.InvariantCulture) },
                        { "Ses", RunOnLogonDesktop ? "Logo" : RunOnScrSaverDesktop ? "ScrS" : "User" },
                    });
        ...
        internal static void Log(Exception e)
        {
            if (e is not KnownException)
            {
                ...
                if (exceptionCount < MaxLogExceptionPerHour)
                {
                    ...
                    TelemetryClient logTelemetry = TelemetryForLog;
                    int logCount = LogCounter.AddOrUpdate(exText, 1, (key, value) => value + 1);

                    if (logTelemetry != null)
                    {
                        if (RepeatedLogIndexSelection.Contains(logCount))
                        {
                            logTelemetry.TrackException(e, new Dictionary<string, string> { { "Count", logCount.ToString(CultureInfo.InvariantCulture) } });
                        }
                    }
                }
                else if (exceptionCount != short.MaxValue)
                {
                    ...
                    TelemetryForEvent?.TrackEvent("MaxLog");
                }
            }
        }
        ...
        private const string HeaderRECV =
        ...
        internal static void Log(string log, bool clearLog = false)
        {
            ...
            lock (AllLogsLock)
            {
                ...
                allLogsIndex = (allLogsIndex + 1) % MAXLOG;
            }
        }
        ...
        internal static void LogAll()
        {
            ...
            if (!p1.Equals(PackageSent))
            {
                log = string.Format(
                    ...
                    PackageSent.Hello,
                    PackageSent.Matrix,
                    PackageSent.Clipboardtxt,
                    PackageSent.Clipboardimg,
                    PackageSent.Byebye,
                    PackageSent.Clipboard,
                    PackageSent.Clipboarddd,
                    PackageSent.Clipboardddend,
                    PackageSent.Explorerdd,
                    inputEventCount,
                    PackageSent.Nil);
                Log(log);
