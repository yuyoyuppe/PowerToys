...
[module: SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes", Scope = "member", Target = "MouseWithoutBorders.Common.#CreateLowIntergrityProcess(System.String,System.String,System.Int32)", Justification = "Dotnet port with style preservation")]
...
[module: SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes", Scope = "member", Target = "MouseWithoutBorders.Common.#CreateLowIntergrityProcess(System.String,System.String,System.Int32,System.Boolean,System.Int64)", Justification = "Dotnet port with style preservation")]
...
namespace MouseWithoutBorders
{
    internal partial class Common
    {
        ...
        internal const int JUST_GOT_BACK_FROM_SCREENSAVER = 9999;
        ...
        internal static Mutex SocketMutex { get; set; } // Synchronization bettween MouseWithoutBorderss running in diffirent desktops
        ...
        internal static void ReleaseSocketMutex()
        {
            if (SocketMutex != null)
            {
                Log("SOCKET MUTEX BEGIN RELEASE.");
                ...
                Log("SOCKET MUTEX RELEASED.");
            }
            else
            {
                Log("SOCKET MUTEX NULL.");
            }
        }
        ...
        internal static bool ExecuteAndTrace(string actionName, Action action, TimeSpan timeout, bool restart = false)
        {
            ...
            if (restart)
            {
                 ...
                 * There is no good wordaround for it so far, still working with [Winsock 2.0 Discussions] to address the issue.
                 * */
                new Thread(
                    () =>
                {
                    ...
                    Log($"Started on desktop {desktop}");

                    Thread.SuppendAllThreadsBut(Thread.CurrentThread.ManagedThreadId);
        ...
        internal static void DoSomthingInTheInputSimulationThread(ThreadStart target)
        {
            /*
            ...
            // SendInput is thread-safe but few users seem to hit a deadlock occasionaly, probably a Windows bug.
        ...
        {
            SendPackage(ID.ALL, initial && Common.GeneratedKey ? PackageType.Heartbeatex : PackageType.Heartbeat);
        }
        ...
        {
            Log($"SendNextMachine: Host machine: {hostMachine}, Next machine: {nextMachine}, Requested XY: {requestedXY}");

            if (GetTick() - lastSendNextMachine < 100)
            {
                Log("Machine switching in progress."); // "Move Mouse relatively" mode, slow machine/network, quick/busy hand.
                return;
            }
            ...
            package.Md.WDelta = (int)nextMachine;

            SkSend(package, null, false);

            Log("SendNextMachine done.");
        }
        ...
        private static void PokeMyself()
        {
            ...
            {
                x = Ran.Next(-9, 10);
                InputSimu.MoveMouseRelative(x, y);
                Thread.Sleep(50);
                InputSimu.MoveMouseRelative(-x, -y);
                Thread.Sleep(50);
        ...
        {
            Common.Log($"{nameof(SetToggleIcon)}: {toggleIcons?.FirstOrDefault()}");
        ...
        internal static void PrepareScreenCapture()
        {
            Common.DoSomethingInUIThread(() =>
            {
                ...
                {
                    Common.MMSleep(0.2);
                    InputSimu.SendKey(new KEYBDDATA() { WVk = (int)VK.SNAPSHOT });
                    InputSimu.SendKey(new KEYBDDATA() { dwFlags = (int)Common.LLKHF.UP, WVk = (int)VK.SNAPSHOT });

                    Common.Log("PrepareScreenCapture: SNAPSHOT simulated.");
        ...
        internal static void OpenImage(string file)
        {
            ...
            // CreateNormalIntergrityProcess(Environment.ExpandEnvironmentVariables(@"%SystemRoot%\System32\Mspaint.exe") +
            // " \"" + file + "\"");
        ...
        internal static void ToggleShowTopMostMessage(string text, string bigText, int timeOut)
        {
            DoSomethingInUIThread(() =>
            {
                ...
                {
                    FrmMessage curentMessageForm = topMostMessageForm;
                    topMostMessageForm = null;
                    curentMessageForm.Close();
                }
            });
        }
        ...
        private static long maxSendTime = 0;
        private static long totalSendCount = 0;
        private static long totalSendTime = 0;
#endif
        ...
        internal static void SkSend(DATA data, uint? exceptDes, bool includeHandShakingSockets)
        {
            ...
            if (sk != null)
            {
                ...
                try
                {
                    ...
                    {
                        Log("********** No active connection found for the remote machine! **********" + data.Des.ToString());
                ...
                lock (sk.TcpSocketsLock)
                {
                    if (sk.TcpSockets != null)
                    {
                        ...
                        foreach (TcpSk t in sk.TcpSockets)
                        {
                            ...
                            {
                                Log("CloseAnUnusedSocket: " + t.MachineName + ":" + t.MachineId + "|" + t.Status.ToString());
                                tobeRemoved = t;
        ...
        internal static bool AtLeastOneSocketConnected()
        {
                ...
                lock (sk.TcpSocketsLock)
                {
                    if (sk.TcpSockets != null)
                    {
                        foreach (TcpSk t in sk.TcpSockets)
                        {
                            if (t.Status == SocketStatus.Connected)
                            {
                                Log("AtLeastOneSocketConnected returning true: " + t.MachineName);
                                return true;
                            }
                        }
                    }
            ...
            Log("AtLeastOneSocketConnected returning false.");
            return false;
        }
        ...
        internal static Socket AtLeastOneServerSocketConnected()
        {
                ...
                lock (sk.TcpSocketsLock)
                {
                    if (sk.TcpSockets != null)
                    {
                        foreach (TcpSk t in sk.TcpSockets)
                        {
                            ...
                            {
                                Log("AtLeastOneServerSocketConnected returning true: " + t.MachineName);
            ...
            Log("AtLeastOneServerSocketConnected returning false.");
            return null;
        }
        ...
        internal static bool AtLeastOneSocketEstablished()
        {
                    ...
                    if (sk.TcpSockets != null)
                    {
                        foreach (TcpSk t in sk.TcpSockets)
                        {
                            if (t.BackingSocket != null && t.BackingSocket.Connected)
                            {
                                if (TestSend(t))
                                {
                                    Log($"{nameof(AtLeastOneSocketEstablished)} returning true: {t.MachineName}");
                                    return true;
                                }
                            }
                        }
            ...
            Log($"{nameof(AtLeastOneSocketEstablished)} returning false.");
            return false;
        }
        ...
        internal static void UpdateSetupMachineMatrix(string desMachine)
        {
            ...
            if (machineCt < 2 && Common.Settings != null && (Common.Settings.GetCurrentPage() is SetupPage1 || Common.Settings.GetCurrentPage() is SetupPage2b))
            {
                ...
                Common.Log("UpdateSetupMachineMatrix: " + string.Join(",", Common.MachineMatrix));
        ...
        internal static void GetMachineName()
        {
            ...
            Log($"========== {nameof(GetMachineName)} ended!");
        }
        ...
        internal static void HideMouseCursor(bool byHideMouseMessage)
        {
            ...
            if ((desMachineID != MachineID && desMachineID != ID.ALL) || byHideMouseMessage)
            {
                ...
                Log($"+++++ HideMouseCursor, byHideMouseMessage = {byHideMouseMessage}");
            }
        ...
        internal static string GetText(IntPtr hWnd)
        {
            ...
            Log("GetWindowText returned " + rv.ToString(CultureInfo.CurrentCulture));
            return sb.ToString();
        }
        ...
        internal static void SendOrReceiveARandomDataBlockPerInitialIV(Stream st, bool send = true)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (read != toRead)
                    {
                        Common.Log("Stream has no more data after reading {0} bytes.", read);
                    }
                }
            }
            catch (IOException e)
