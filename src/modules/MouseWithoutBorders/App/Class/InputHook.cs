...
namespace MouseWithoutBorders.Class
{
    internal class InputHook
    {
        ...
        internal event KeybdEvHandler KeydbEvent;
        ...
        private static KeyboardHookStruct keydbHookStruct;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (nCode >= 0 && MouseEvent != null)
                    {
                        ...
                        {
                            Common.Log($"{nameof(SkipMouseUpCount)}: {SkipMouseUpCount}.");
                            SkipMouseUpCount--;
                            rv = NativeMethods.CallNextHookEx(hMouseHook, nCode, wParam, lParam);
                            return rv;
                        }
...
#if DEBUG
                        Common.Log(string.Format("MouseHookProc.MOUSEDATA: ({0}, {1})", hookCallbackMouseData.X, hookCallbackMouseData.Y));
#endif
                        hookCallbackMouseData.dwFlags = wParam;

                        // Use wDelta to store XBUTTON1/XBUTTON2 data.
                        hookCallbackMouseData.WDelta = (short)((mouseHookStruct.MouseData >> 16) & 0xffff);
        ...
        private int KeyboardHookProc(int nCode, int wParam, IntPtr lParam)
        {
            ...
            keydbHookStruct = LParamToKeyboardHookStruct(lParam);
            hookCallbackKeybdData.dwFlags = keydbHookStruct.Flags;
            hookCallbackKeybdData.WVk = (short)keydbHookStruct.VkCode;

            if (nCode >= 0 && KeydbEvent != null)
            {
                if (!ProcessKeyEx(keydbHookStruct.VkCode, keydbHookStruct.Flags, hookCallbackKeybdData))
                {
                    return 1;
                }

                KeydbEvent(hookCallbackKeybdData);
            }
            ...
            if (Common.DesMachineID == ID.NONE || Common.DesMachineID == ID.ALL || Common.DesMachineID == Common.MachineID)
            {
                ...
                {
                    InputSimu.SendKey(hookCallbackKeybdData);
                    return 1;
                }
                else
                {
        ...
        private const int CTRLTOUCH = 4;
        private readonly long[] ctrlTouches = new long[CTRLTOUCH];
        ...
        private bool ProcessKeyEx(int vkCode, int flags, KEYBDDATA hookCallbackKeybdData)
        {
                ...
                switch ((VK)vkCode)
                {
                    ...
                    case VK.RCONTROL:
                        ...
                        if (Setting.Values.HotKeySwitch2AllPC == 1)
                        {
                            ...
                            ctrlTouchesUpIndex = ((ctrlTouchesUpIndex % CTRLTOUCH) + 2) % CTRLTOUCH;
                        }
            ...
            else
            {
                ...
                switch ((VK)vkCode)
                {
                    ...
                    case VK.LCONTROL:
                    case VK.RCONTROL:
                        Common.Log("VK.RCONTROL");
                        CtrlDown = true;

                        if (Setting.Values.HotKeySwitch2AllPC == 1)
                        {
                            ctrlTouches[ctrlTouchesDnIndex] = Common.GetTick();
                            ctrlTouchesDnIndex = (ctrlTouchesDnIndex + 2) % CTRLTOUCH;
                        }

                        allTouched = true;

                        for (int i = 0; i < CTRLTOUCH; i++)
                        {
                        ...
                        if (allTouched && Common.GetTick() - Common.IJustGotAKey > 1000)
                        {
                            ResetLastSwitchKeys();
                    ...
                    case VK.DELETE:
                        if (CtrlDown && altDown)
                        {
                           ...
                           Common.DoSomthingInUIThread(delegate()
                           {
