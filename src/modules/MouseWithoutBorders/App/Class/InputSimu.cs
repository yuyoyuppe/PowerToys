...
[module: SuppressMessage("Microsoft.Portability", "CA1901:PInvokeDeclarationsShouldBePortable", Scope = "member", Target = "MouseWithoutBorders.InputSimu.#keybd_event(System.Byte,System.Byte,System.UInt32,System.Int32)", MessageId = "3", Justification = "Dotnet port with style preservation")]
[module: SuppressMessage("Microsoft.Usage", "CA1806:DoNotIgnoreMethodResults", Scope = "member", Target = "MouseWithoutBorders.InputSimu.#InputProcessKeyEx(System.Int32,System.Int32,System.Boolean&)", MessageId = "MouseWithoutBorders.NativeMethods.LockWorkStation", Justification = "Dotnet port with style preservation")]
    ...
    internal class InputSimu
    {
        ...
        private static uint SendInputEx(NativeMethods.INPUT input)
        {
            ...
            if (Common.Is64bitOS)
            {
                ...
                {
                    rv = NativeMethods.InjectMouseInput(new[] { input64.mi }, 1);
                }
                else
                {
            ...
            else
            {
                if (input.type == 0 && (input.mi.dwFlags & (int)NativeMethods.MOUSEEVENTF.MOVE) != 0 && NativeMethods.InjectMouseInputAvailable)
                {
                    rv = NativeMethods.InjectMouseInput(new[] { input.mi }, 1);
                }
                else
                {
        ...
        internal static uint SendMouse(MOUSEDATA md)
        {
            ...
            mouse_input.mi.mouseData = md.WDelta;

#if DEBUG
            if (md.dwFlags != Common.WM_MOUSEMOVE)
            {
                Common.Log($"InputSimu.SendMouse: x = {md.X}, y = {md.Y}, wDelta = {md.WDelta}, dwFlags = {md.dwFlags}.");
            }
#endif
...
#if DEBUG
            Common.Log($"InputSimu.MoveMouseEx: x = {x}, y = {y}.");
#endif
...
#if DEBUG
            Common.Log($"InputSimu.MoveMouse: x = {x}, y = {y}.");
#endif
...
#if DEBUG
            Common.Log($"InputSimu.MoveMouseRelative: x = {dx}, y = {dy}.");
#endif
                ...
                switch ((VK)vkCode)
                {
                    ...
                    case (VK)'L':
                        if (winDown)
                        {
                            ...
                            Common.Log("LockWorkStation returned " + rv.ToString(CultureInfo.CurrentCulture));
                        }
