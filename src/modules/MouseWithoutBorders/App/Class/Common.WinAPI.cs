...
namespace MouseWithoutBorders
{
    ...
    internal partial class Common
    {
        ...
        internal static void GetScreenConfig()
        {
            try
            {
                Common.Log("==================== GetScreenConfig started");
        ...
        private static bool CloseScreenSaverFunc(IntPtr hWnd, IntPtr lParam)
        {
            ...
            {
                Log("Posting WM_CLOSE to " + hWnd.ToString(CultureInfo.InvariantCulture));
        ...
        internal static void StartMMService(string desktopToRunMMOn)
        {
            if (!Common.RunWithNoAdminRight)
            {
                Log("*** Starting on active Desktop: " + desktopToRunMMOn);
                StartMouseWithoutBordersService(desktopToRunMMOn);
            }
        }
        ...
        internal static void CheckForDesktopSwitchEvent(bool cleanupIfExit)
        {
            try
            {
                if (!IsMyDesktopActive() || Common.CurrentProcess.SessionId != NativeMethods.WTSGetActiveConsoleSessionId())
                {
                    ...
                    Log("*** Active Desktop = " + activeDesktop);
                    Log("*** My Desktop = " + myDesktop);

                    if (myDesktop.Equals(activeDesktop, StringComparison.OrdinalIgnoreCase))
                    {
                        Log("*** Active Desktop == My Desktop (TS session)");
                    }
                    ...
                    {
                        Log("*** Desktop inactive, exiting: " + myDesktop);
                        Setting.Values.LastX = JUST_GOT_BACK_FROM_SCREENSAVER;
                        if (cleanupIfExit)
                        {
                            Common.Cleanup();
                        }
