...
namespace MouseWithoutBorders.Class
{
    ...
    internal class SocketStuff
    {
        ...
        {
            Common.Log("SocketStuff started.");

            bASE_PORT = port;
            Common.Ran = new Random();

            Common.Log("Validating session...");
            ...
            Common.Log("Creating socket list and mutex...");
            ...
            // JUST_GOT_BACK_FROM_SCREENSAVER: For bug: monitor does not turn off after logon screen saver exits
            else if (!Common.RunOnScrSaverDesktop)
            {
                if (Setting.Values.LastX == Common.JUST_GOT_BACK_FROM_SCREENSAVER)
                {
        ...
        internal void Close(bool sentWait)
        {
            try
            {
                ...
                _ = Common.ExecuteAndTrace(
                    ...
                    () =>
                    {
                        Common.Log($"Closing socket [{skMessageServer?.Name}].");
                        skMessageServer?.Close(); // The original ones, not the socket instances proceduced by the accept() method.
                        skMessageServer = null;

                        Common.Log($"Closing socket [{skClipboardServer?.Name}].");
                        ...
                        try
                        {
                            ...
                            lock (TcpSocketsLock)
                            {
                                ...
                                if (TcpSockets != null)
                                {
                                    Common.Log("********** Closing Sockets: " + TcpSockets.Count.ToString(CultureInfo.InvariantCulture));
                                ...
                                Common.Log("********** Sockets Closed: " + c.ToString(CultureInfo.CurrentCulture));
                            }
                        }
                        catch (Exception e)
                        {
        ...
        private static int TcpSendData(TcpSk tcp, byte[] bytes)
        {
            ...
            bytes[1] = 0;
            for (int i = 2; i < Common.PACKAGESIZE; i++)
            {
        ...
        internal static DATA TcpRecvData(TcpSk tcp, out int bytesReceived)
        {
            ...
            {
                string log = $"{nameof(TcpRecvData)}: The socket is no longer connected, it could have been closed by the remote host.";
                Common.Log(log);
                throw new ExpectedSocketException(log);
            }
            ...
            try
            {
                bytesReceived = decryptedStream.ReadEx(buf, 0, Common.PACKAGESIZE);

                if (bytesReceived != Common.PACKAGESIZE)
                {
                ...
                {
                    bytesReceived = decryptedStream.ReadEx(buf, Common.PACKAGESIZE, Common.PACKAGESIZE);
            ...
            {
                string log = $"{nameof(TcpRecvData)}: Exception reading from the socket: {tcp.MachineName}: {e.InnerException?.GetType()}/{e.Message}. (This is expected when the remote machine closes the connection during desktop switch or reconnection.)";
                Common.Log(log);
        ...
        private static void PreProcessData(PackageType type)
        {
            switch (type)
            {
                ...
                default:
                    ...
                    switch (subtype)
                    {
                        case (byte)PackageType.Clipboardtxt:
                            Common.PackageSent.Clipboardtxt++;
                            break;

                        case (byte)PackageType.Clipboardimg:
                            Common.PackageSent.Clipboardimg++;
                            break;
        ...
        private void TCPServerThread(object param)
        {
            try
            {
                ...
                do
                {
                    Common.Log("TCPServerThread: Waiting for request...");
        ...
        private static string GetMachineNameFromSocket(Socket socket)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                ...
                if (dotPos > 0)
                {
                    Common.Log("Removing domain part from the full machine name: {0}.", name);
                    name = name[..dotPos];
                }
            }
        ...
        internal void UpdateTCPClients()
        {
            ...
            Common.Log("!!!!! UpdateTCPClients !!!!!");

            try
            {
                if (Common.MachineMatrix != null)
                {
                    Common.Log("MachineMatrix = " + string.Join(", ", Common.MachineMatrix));
                    ...
                    foreach (string st in Common.MachineMatrix)
                    {
                            ...
                            !machineName.Equals(Common.MachineName.Trim(), StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if (found)
                            {
                                Common.Log(machineName + " is already connected! ^^^^^^^^^^^^^^^^^^^^^");
                                continue;
                            }
        ...
        internal void StartNewTcpClient(string machineName)
        {
            void ClientThread(object obj)
            {
                ...
                Common.Log("Connecting to: " + machineName);
                ...
                if (!string.IsNullOrEmpty(Setting.Values.Name2IP))
                {
                    ...
                    if (validatedAddresses.Count > 0)
                    {
                        ...
                        Common.Log("Using both user-defined Name-to-IP mappings and DNS result for " + machineName);
        ...
        private bool CheckForSameSubNet(List<IPAddress> validatedAddresses, string machineName)
        {
            ...
            // Only support if IPv4 addrs found in both.
            IEnumerable<IPAddress> remoteIPv4Addrs = validatedAddresses.Where(addr => addr?.AddressFamily == AddressFamily.InterNetwork);

            if (!remoteIPv4Addrs.Any())
            {
                Common.Log($"No IPv4 resolved from the remote machine: {machineName}.");
                return true;
            }

            List<IPAddress> localIPv4Addrs = GetMyIPv4Addresses().ToList();

            if (!localIPv4Addrs.Any())
            {
                Common.Log($"No IPv4 resolved from the local machine: {Common.MachineName}");
                return true;
            }

            foreach (IPAddress remote in remoteIPv4Addrs)
            {
                foreach (IPAddress local in localIPv4Addrs)
                {
        ...
        private IEnumerable<IPAddress> GetMyIPv4Addresses()
        {
            try
            {
                IEnumerable<IPAddress> ip4addrs = NetworkInterface.GetAllNetworkInterfaces()?
        ...
        private void StartNewTcpClientThread(string machineName, IPAddress ip)
        {
            void NewTcpClient()
            {
                ...
                try
                {
                    ...
                    {
                        Common.Log(machineName + " is already connected by another client socket.");
                        return;
                    }

                    if (Common.IsConnectingByAClientSocketTo(machineName, ip))
                    {
                        Common.Log($"{machineName}:{ip} is already being connected by another client socket.");
                        return;
                    }

                    TcpSk tcp = AddTcpSocket(true, tcpClient.Client, SocketStatus.Connecting, machineName, ip);

                    // Update the other server socket's machine name based on this coresponding client socket.
                    UpdateTcpSockets(tcp, SocketStatus.Connecting);

                    Common.Log(string.Format(CultureInfo.CurrentCulture, "=====> Connecting to: {0}:{1}", machineName, ip.ToString()));
                    ...
                    do
                    {
                        ...
                        catch (ObjectDisposedException)
                        {
                            ...
                            Common.Log($"tcpClient.Connect: The socket has already been disposed: {machineName}:{ip}");
                            return;
                        }
                        catch (SocketException e)
                        {
                            ...
                            if (timeoutLeft > 0)
                            {
                                Common.Log($"tcpClient.Connect: {timeoutLeft}: {e.Message}");
                                Thread.Sleep(1000);
                                continue;
                            }
                            else
                    ...
                    Common.Log($"=====> Connected: {tcpClient.Client.LocalEndPoint} => {machineName}: {ip}");
        ...
        private void MainTCPRoutine(TcpSk tcp, string machineName, bool isClient)
        {
            ...
            byte[] buf = RandomNumberGenerator.GetBytes(Common.PACKAGESIZEEX);
            d = new DATA(buf);
            ...
            {
                Common.Log($"{nameof(MainTCPRoutine)}: The socket could have been closed/disposed by other threads.");
                return;
            }

            try
            {
                currentSocket.SendBufferSize = Common.PACKAGESIZE * 10000;
                currentSocket.ReceiveBufferSize = Common.PACKAGESIZE * 10000;
                currentSocket.NoDelay = true; // This is very intersting to know:(
                ...
                Common.Log($"MainTCPRoutine: Remote machineName/IP = {remoteMachine}/{strIP}");
            }
            catch (ObjectDisposedException e)
            {
                Common.PleaseReopenSocket = Common.REOPEN_WHEN_WSAECONNRESET;
            ...
            while (true)
            {
                try
                {
                    DATA package = TcpRecvData(currentTcp, out int receivedCount);
                    ...
                    else
                    {
                        if (packageCount >= 0)
                        {
                            ...
                            else if (package.Type == PackageType.Handshakeack)
                            {
                                   ...
                                   package.Machine3 == d.Machine3 && package.Machine4 == d.Machine4)
                                {
                                    ...
                                    {
                                        Common.Log($"DNS.RemoteMachineName({remoteMachine}) <> Claimed.MachineName({claimedMachineName}), using the claimed machine name.");
                                    ...
                                    {
                                        Common.Log("=====> Duplicate connected client socket for: " + remoteMachine + ":" + strIP + " is being removed.");
                                        UpdateTcpSockets(currentTcp, SocketStatus.Forceclosed);
                                        currentSocket.Close();
                                        return;
                                    }
                                    ...
                                    {
                                        Common.Log("Connected to/from local socket: " + strIP + (isClient ? "-Client" : "-Server"));
                                    ...
                                    Common.Log("))))))))))))))) Machine got trusted: " + remoteMachine + ":" + strIP + ", Is client: " + isClient);
                                    ...
                                    {
                                        Common.Log("Machine updated: " + remoteMachine + "/" + remoteID.ToString());

                                        if (machineInfo.Name.Equals(Common.DesMachineName, StringComparison.OrdinalIgnoreCase))
                                        {
                                            Common.Log("Des ID updated: " + Common.DesMachineID.ToString() +
                                                "/" + remoteID.ToString());
                                    ...
                                    {
                                        Common.Log("New machine conneceted: {0}.", remoteMachine);
                                ...
                                {
                                    Common.Log("Invalid ACK from " + remoteMachine);
                                    UpdateTcpSockets(currentTcp, SocketStatus.Invalidkey);
                            ...
                            else if (package.Type is PackageType.Heartbeat or PackageType.Heartbeatex)
                            {
                                if (packageCount > 5)
                                {
                                    packageCount--;
                        ...
                        else if (receivedCount > 0)
                        {
                            ...
                            if (lastRemoteMachineID != (long)remoteID)
                            {
                                ...
                                Common.Log($"MainTCPRoutine: Remote machine = {strIP}/{lastRemoteMachineID}");
                            }

                            if (package.Type == PackageType.Handshakeack)
                            {
                                Common.Log("Skipping the rest of the Handshake packages.");
                            }
                            else
                            {
        ...
        private static void AcceptConnectionAndSendClipboardData(object param)
        {
            ...
            do
            {
                Common.Log("SendClipboardData: Waiting for request...");
                Socket s = null;
        ...
        internal static void SendOrReceiveClipboardData(Socket s)
        {
            try
            {
                ...
                Common.Log("SendClipboardData: Request accepted: " + s.LocalEndPoint.ToString() + "/" + remoteEndPoint);
                Common.IsDroping = false;
                ...
                {
                    Common.Log($"{nameof(SendOrReceiveClipboardData)}: Clipboard connection accepted: " + remoteEndPoint);
        ...
        internal static void SendClipboardData(Socket s, Stream ecStream)
        {
            ...
            else if (!Common.IsClipboardDataImage && Common.LastClipboardData != null)
            {
                try
                {
                    ...
                    if (data != null)
                    {
                        ...
                        Common.Log("Text sent: " + data.Length.ToString(CultureInfo.CurrentCulture));
                    }
            ...
            else if (Common.LastClipboardData != null && Common.LastClipboardData.Length > 0)
            {
                ...
                try
                {
                    ecStream.Write(header, 0, header.Length);
                    _ = SendData(s, ecStream, data);
                    Common.Log("Image sent: " + data.Length.ToString(CultureInfo.CurrentCulture));
                    s.Close(CLOSE_TIMEOUT);
                }
                catch (IOException e)
                {
        ...
        private static bool SendFileEx(Socket s, Stream ecStream, string fileName)
        {
            try
            {
                using (FileStream f = File.OpenRead(fileName))
                {
                    ...
                    if ((rv = Common.PACKAGESIZE - (sentCount % Common.PACKAGESIZE)) > 0)
                    {
        ...
        private static bool SendData(Socket s, Stream ecStream, byte[] data)
        {
            ...
            try
            {
                ...
                if ((rv = sentCount % Common.PACKAGESIZE) > 0)
                {
                    Array.Clear(buf, 0, buf.Length);
                    ecStream.Write(buf, 0, rv);
                }
                ...
                ecStream.Flush();
                Common.Log("Data sent: " + data.Length.ToString(CultureInfo.InvariantCulture));
                r = true;
            }
            catch (Exception e)
            {
                    ...
                    if (TcpSockets != null)
                    {
                        if (status == SocketStatus.Connected && tcp.IsClient)
                        {
                            ...
                            foreach (TcpSk t in tobeRemovedSockets)
                            {
                                ...
                                Common.Log($"Closing duplicated socket {t.MachineName}: {t.Address}");
                            }
                        }
                        ...
                        foreach (TcpSk t in TcpSockets)
                        {
                            if (t.Status is SocketStatus.Error or
                                ...
                                // t.status == SocketStatus.invalidkey ||
                                SocketStatus.NA or
                        ...
                        if (tcp != null)
                        {
                            ...
                            {
                                // Update the socket's machine name based on its coresponding client/server socket.
                                foreach (TcpSk t in TcpSockets)
                                {
        ...
        private void PreventDoS(List<TcpSk> sockets)
        {
            if (sockets.Count > 100)
            {
                ...
                try
                {
                    ...
                    _ = Common.CreateLowIntergrityProcess(
