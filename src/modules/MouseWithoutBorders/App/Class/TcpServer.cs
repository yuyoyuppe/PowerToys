                    ...
                    try
                    {
                        ...
                        if (proc.ExitCode == 0)
                        {
                            ...
                            foreach (string portLogLine in portLog)
                            {
                                ...
                                {
                                    Common.TelemetryForLog?.TrackException(e);
                                }

                                Common.TelemetryForLog?.TrackTrace($"{log}: {portLogLine}\t{process?.ProcessName}. My process id: {Common.CurrentProcess.Id}");
                            }
                        }
                        else
                        {
                            Common.TelemetryLogTrace($"Error running netstat with exit code {proc.ExitCode}, console output:\r\n {status}.", SeverityLevel.Warning);
                        }
                    }
                    catch (Exception e)
                    {
                        Common.TelemetryLogTrace($"Exception running netstat: {e.Message}", SeverityLevel.Warning);
                    }
                });
