...
namespace MouseWithoutBorders
{
    internal partial class Common
    {
        ...
        internal static void Init()
        {
            ...
            try
            {
                InputSimu.MoveMouseRelative(0, 0);
            }
            ...
            bool dummy = Setting.Values.DrawMouseEx; // Reading from registry.
            Is64bitOS = IntPtr.Size == 8;
            tcpPort = Setting.Values.TcpPort;
            GetScreenConfig();
            PackageSent = new PackageMonitor(0);
            PackageRecv = new PackageMonitor(0);
            ...
            TelemetryForEvent?.TrackMetric(new MetricTelemetry("Matrix", ((double?)Common.MachineMatrix?.Where(name => !string.IsNullOrWhiteSpace(name)).Count()).Value));
        }
        ...
        {
            Log("NetworkAvailabilityEventArgs.IsAvailabe: " + e.IsAvailable.ToString(CultureInfo.InvariantCulture));
            Common.WndProcCounter++;
