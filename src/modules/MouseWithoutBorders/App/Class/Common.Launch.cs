...
namespace MouseWithoutBorders
{
    internal partial class Common
    {
        ...
        internal static bool ImpersonateLoggedOnUserAndDoSomething(Action targetFunc)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    Log("WTSQueryUserToken returned " + rv.ToString(CultureInfo.CurrentCulture));

                    if (rv == 0)
                    {
                        Log($"WTSQueryUserToken failed with: {Marshal.GetLastWin32Error()}.");
                        return false;
                    }
        ...
        internal static int CreateProcessInInputDesktopSession(string commandLine, string arg, string desktop, short wShowWindow, bool lowIntergrity = false)

        // As user who runs explorer.exe
        {
            if (!Program.User.ToLower().Contains("system"))
            {
            ...
            Common.Log("CreateProcessInInputDesktopSession called, launching " + commandLineWithArg + " on " + desktop);
            ...
            try
            {
                ...
                Log("CreateProcessAsUser returned " + iResultOfCreateProcessAsUser.ToString(CultureInfo.CurrentCulture));
        ...
        internal static bool CreateLowIntergrityProcess(string commandLine, string args, int wait, bool killIfTimedOut, long limitedMem, short wShowWindow = 0)
        {
            ...
            if (wait > 0)
            {
                if (limitedMem > 0)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            Log(string.Format(CultureInfo.CurrentCulture, "Processs log (mem): {0}, {1}", sec, p.PrivateMemorySize64));
                            return false;
                        }
            ...
            if (processID > 0 && hProcess.ToInt32() > 0)
            {
                ...
                foreach (Process p in processes)
                {
                    if (p.SessionId == dwSessionId)
                    {
                        ...
                        try
                        {
                            if (NativeMethods.NtQueryInformationProcess(p.Handle, 0, ref pbi, (uint)Marshal.SizeOf(pbi), out uint bytesWritten) >= 0)
                            {// NT_SUCCESS(...)
