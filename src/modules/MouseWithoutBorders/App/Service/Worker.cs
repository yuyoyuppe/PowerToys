    ...
    internal sealed class Worker : IHostedService
    {
        ...
        private int RunMeUnderSystemAccount(int noOfTry, string activeDesktop, string userLocalAppDataPath)
        {
            ...
            try
            {
                ...
                {
                    waitCount--;
                    Log("The session is detached/attached.");
                    Thread.Sleep(500);
                }
                ...
                {
                    Log($"Executing {me} on [{activeDesktop}], {NativeMethods.WTSGetActiveConsoleSessionId()}");
                ...
                else
                {
                    ...
                    {
                        Thread.Sleep(1000);
                        Log($"Executing {me} default, {(appSessionId >= 0 ? appSessionId : (int)NativeMethods.WTSGetActiveConsoleSessionId())}");
                        ...
                        Thread.Sleep(1000);
                        Log("Executing " + me + " \"winlogon\" on session " + appSessionId.ToString(CultureInfo.InvariantCulture));
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                expectedRunCount = 2;
                Log($"successRunCount = {successRunCount}, processCount = {processCount}");
                ...
                if (isLoggingOff || isByWindows)
                {
                    ...
                    while ((successRunCount < expectedRunCount || processCount < expectedRunCount) && c++ < 30)
                    {
                        ...
                        Log($"successRunCount(2) = {successRunCount}, processCount = {processCount}");
                    }
                    ...
                    {
                        Log("Found none or one process, one more try...");
                        successRunCount = RunMeUnderSystemAccount(noOfTry, activeDesktop, userLocalAppDataPath);
                        Thread.Sleep(5000);
                        p = Process.GetProcessesByName(processName);
                        processCount = p != null ? p.Length : 0;
                        Log($"successRunCount(3) = {successRunCount}, processCount = {processCount}");
                    }
                }
                else
                {
