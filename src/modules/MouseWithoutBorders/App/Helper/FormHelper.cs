...
namespace MouseWithoutBorders
{
    public partial class FormHelper : System.Windows.Forms.Form
    {
        ...
        private const int MAXIMAGESIZE = 50 * 1024 * 1024;
        ...
        private void QuitDueToCommunicationError()
        {
            if (!quitDueToCommunicationError)
            {
                ...
                {
                    // TODO(@yuyoyuppe): not using IPC -> no need for this warning
                    // MessageBox.Show("Unable to connect to Mouse Without Borders process, clipboard sharing is no longer working.\r\nSee EventLog for more information.",
        ...
        private string GetClipboardText()
        {
            ...
            {
                st += "HTM" + tmp + TEXTTYPESEP;
            }

            if (st.Length > 0)
            {
                if (st.Length > MAXTEXTSIZE)
                {
                    st = null;
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            switch (m.Msg)
            {
                ...
                case SharedConst.QUIT_CMD:
                    // TODO(@yuyoyuppe): we shouldn't kill current process now that they're the same
                    // Process.GetCurrentProcess().Kill();
                    break;
                    ...
                    && stop.X - start.X > 0 && stop.Y - start.Y > 0)
                {
                    ...
                    try
                    {
                        ...
                        if (rightMouseButton)
                        {
                            ...
                            {
                                SendLog("IO!Exception!: " + ioe.Message);
                            }
                            catch (ArgumentNullException arge)
                            {
                                SendLog("ArgNull!Exception!: " + arge.Message);
                            }
                            catch (ExternalException ie)
                            {
                                SendLog("Internal!Exception!: " + ie.Message);
                            }
                        }
                        else
                        {
